...
pushd "$glean_as"
patch -p1 --no-backup-if-mismatch --quiet < "$patches/upstream-patches/glean-gradle-fixes.patch"
...
patch -p1 --no-backup-if-mismatch --quiet < "$patches/upstream-patches/ac-fix-config.patch"
...
cat "$patches/preferences/userjs-01-pyllyukko.js" >> mobile/android/app/mobile.js
